    ...
    "rename the graph column of quads when inside a GRAPH statement" when {
      ...
      """
        ...
        Fix.un(renamed) match {
          case Project(_, Project(_, Scan(graph, BGP(quads)))) =>
            assertForAllQuads(quads)(_.g shouldEqual URIVAL(graph))
          case _ => fail
        }
      }
      ...
      """
        ...
        Fix.un(renamed) match {
          case Project(
                ...
                Project(
                  _,
                  Join(BGP(externalQuads), Scan(graph, BGP(internalQuads)))
                )
              ) =>
            assertForAllQuads(internalQuads)(_.g shouldEqual URIVAL(graph))
            assertForAllQuads(externalQuads)(_.g shouldEqual GRAPH_VARIABLE)
          case _ => fail
        }
      }

      "has BGP before and Union after Scan" ignore {
        ...
        Fix.un(renamed) match {
          case Project(
            ...
            assertForAllQuads(leftInsideQuads.concat(rightInsideQuads))(
              _.g shouldEqual graph
            )
          case _ => fail
        }
      }

      "has BGP before and LeftJoin after Scan" ignore {
      ...
      "has BGP before and a Join with Scan after first Scan" ignore {
      ...
      """
        ...
        Fix.un(renamed) match {
          case Project(
            ...
            assertForAllQuads(externalQuads)(_.g shouldEqual GRAPH_VARIABLE)
            assertForAllQuads(graph1Quads)(_.g shouldEqual graph1)
            assertForAllQuads(graph2Quads)(_.g shouldEqual graph2)
          case _ => fail
        }
      }
    }
